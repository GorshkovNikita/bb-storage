...
// as a backend.
func NewCloudBlobAccess(bucket *blob.Bucket, keyPrefix string, readBufferFactory blobstore.ReadBufferFactory, digestKeyFormat digest.KeyFormat, beforeCopy BeforeCopyFunc, capabilitiesProvider capabilities.Provider) blobstore.BlobAccess {
...
func (ba *cloudBlobAccess) Get(ctx context.Context, digest digest.Digest) buffer.Buffer {
	...
	b := ba.readBufferFactory.NewBufferFromReader(
		digest,
		result,
...
func (ba *cloudBlobAccess) GetFromComposite(ctx context.Context, parentDigest, childDigest digest.Digest, slicer slicing.BlobSlicer) buffer.Buffer {
